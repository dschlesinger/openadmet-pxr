...
def main() -> None:
    ...
    path = _DEFAULT_RESULTS_DIR / args.results

    if not args.results or not path.exists():
        raise argparse.ArgumentError(None, "Please provide the location of a csv to validate")

    success, fails = validate_activity_submission(path)

    if success:
        print("\033[32mSubmission is valid\033[0m")
    else:
        print("\033[31mSubmission failed\033[0m")
        for f in fails:

...
Usage:
    ...
    python scripts/finetune_chemeleon.py --train data/train.csv --test data/test.csv --out results/chemeleon_submission.csv
    python scripts/finetune_chemeleon.py --epochs 30 --val-split 0.1 --batch-size 64
"""
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("results/chemeleon_submission.csv"))
    ...
    all_datapoints = _build_datapoints(train_smiles, train_targets)
    mols = [dp.mol for dp in all_datapoints]

    train_size = 1.0 - args.val_split
    train_indices, val_indices, _ = data.make_split_indices(
        mols, "random", (train_size, args.val_split, 0.0), seed=args.seed
    )
    train_data, val_data, _ = data.split_data_by_indices(all_datapoints, train_indices, val_indices, [])
    ...
    print(f"\nSaved {len(submission)} predictions -> {args.out}")
